	...
	public class AppMain{
		...
		public void Update(){
			...
			if(gpd.ButtonsDown.HasFlag(GamePadButtons.Cross)){
				...
				for(int i = 0; i < 100; i++){
					tm.AddThing(new Thing(360,Globals.frameCount));
				}
			}
		...
		public void Render(){
			...
			if(sw.ElapsedMilliseconds > 1000){
				Console.WriteLine(Globals.COMPUTE_BY.ToString() + " - Amount of Things:" + tm.ThingCount() + " - fps:" + fpsCounter + " Memory usage: " + (GC.GetTotalMemory(true) / 1024) + "KB");
				fpsCounter = 0;
				sw.Reset();
				sw.Start();
			}
		...
		void PackModelVertexBuffer(ref GraphicsContext gc){
			...
			for(int i = 0; i < Globals.things.Count; i++){ 
				amountOfVertex += Globals.things[i].prim.Count;
				if(matrixChanged)
					matrixUniform[i] = Globals.things[i].modelToWorld;
				HowManyWeGot++;
			}
			...
			for(int i = 0; i < Globals.things.Count; i++){ // Now for each, put them into the arrays
				...
				ArrayFillComplex(ref matrixNumber, position, ref number, Globals.things[i].prim.Count);
				sww.Stop();
				//Console.WriteLine("doing inner packing took: " + sww.ElapsedMilliseconds + "ms");
				matrixCounter++;
				primArray[i].Set(DrawMode.Triangles, position, Globals.things[i].prim.Count, 0);
				position += (Globals.things[i].prim.Count);
			}					
			sww.Stop();
