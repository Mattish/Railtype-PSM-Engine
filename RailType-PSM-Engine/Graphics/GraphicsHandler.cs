    ...
    public class GraphicsHandler{
        ...
        public void Register(GraphicThing inputThing){
            if (inputThing.HasModel){
                ...
                Primitive outputPrimitive = new Primitive(DrawMode.Triangles, _indicesIndex,
                    (ushort) model.indices.Length, 0);

                //SetIndicies
                Array.Copy(model.indices, 0, indicestmp, 0, model.indices.Length);
                for (int i = 0; i < indicestmp.Length; i++) { indicestmp[i] += _verticesIndex; }
        ...
        public void PrintInfo(){
            ...
            Console.WriteLine("Fragmented-Vertices:{0:D},Indicies:{1:D}", _fragmentationVerticesCount,
                _fragmentationIndicesCount);
        }
        ...
        public void Draw(Primitive[] primsToDraw, Matrix4[] batchMatricies, int[] batchThingNumbers, int start, int count, int shaderNo, int textureNo){
            _gc.SetVertexBuffer(0, _vb);
            _gc.SetShaderProgram(AssetHandler.GetInstance()
                .GetSPSimple());
            Matrix4 vp = BuildProjectionMatrix(CameraToWorld);
            ...
            AssetHandler.GetInstance()
                .GetSPSimple()
                .SetUniformValue(1, batchMatricies, 0, start, count);
            //ThingNumbers
            AssetHandler.GetInstance()
                .GetSPSimple()
                .SetUniformValue(2, batchThingNumbers, 0, start, count);
            _gc.DrawArrays(primsToDraw, start, count);
        }
        ...
        private void ArrayFillComplex<T>(ref T[] arrayToFill, int destinationIndex, ref T[] fillValue, int count){
            ...
            for (int i = fillValue.Length; i < count; i *= 2){
                int copyLength = i;
                if (i > arrayToFillHalfLength) { copyLength = count - i; }
