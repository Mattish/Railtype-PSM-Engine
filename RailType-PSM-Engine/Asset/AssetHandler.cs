...
namespace RailTypePSMEngine.Asset{
    public class AssetHandler{
        ...
        public void AddTextureCount(string name){
            if (_textureAssetDict.ContainsKey(name)) { _textureAssetDict[name].AddReferenceCount(); }
            else{
        ...
        public void RemoveTextureCount(string name){
            if (_textureAssetDict.ContainsKey(name)){
                ...
                if (_textureAssetDict[name].ReferenceCount() == 0) { _textureAssetDict[name].Unload(); }
            }
        }
